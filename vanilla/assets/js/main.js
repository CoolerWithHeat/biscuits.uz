...
function dynamicPriceUpdate(resulted_number, holder) {
  ...
  if (holder) {
    ...
    for (let times = 0; times <= times_needed; times++) {
      ...
      setTimeout(() => {
        holder.textContent = `$${digit_to_update.toFixed(1)}`;
        if (times === times_needed) {
          holder.textContent = `$${resulted_number.toFixed(1)}`;
          last_resulted = wholePart;
        }
      }, 24 * (times / 2));
    }
  }
...
const dynamicUpdate = debounceLast(dynamicPriceUpdate, 499);
